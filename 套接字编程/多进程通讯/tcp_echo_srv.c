...
int install_sig_handlers(){
	...
	// 函数来处理SIGCHLD信号(复杂些)
	struct sigaction sigact_ign, old_sigact_ign;
	sigact_ign.sa_handler = SIG_IGN;//sig_pipe()，信号处理函数
	sigact_ign.sa_flags = 0;
	sigact_ign.sa_flags |= SA_RESTART;//设置受影响的慢系统调用重启
	sigemptyset(&sigact_ign.sa_mask);
	res = sigaction(SIGCHLD, &sigact_ign, &old_sigact_ign);
	if(res)
		return -2;

	// TODO 安装SIGINT信号处理器,若失败返回-3
	struct sigaction sigact_int, old_sigact_int;
	sigact_int.sa_handler = sig_int;//sig_pipe()，信号处理函数
...
int main(int argc, char* argv[])
{
	...
	if(res == -1)
		printf("install SIGPIPE fail!\n");
	else if(res == -2)
		printf("install SIGCHLD fail!\n");
	else if(res == -3)
	...
	bprintf(fp_res, "[srv](%d) server[%s:%s] is initializing!\n", pid, argv[1], argv[2]);
