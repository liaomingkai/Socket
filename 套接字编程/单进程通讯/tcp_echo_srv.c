...
void echo_rep(int sockfd) {
	...
	count = 0;
         do
        {
            res = read(sockfd, &(buf[count]), len_h-count); // 以下代码紧跟read();
            if (res < 0)
            {
...
int main(int argc, char* argv[])
{
	...
	fprintf(stdout, "[srv] server[%s:%s] is initializing!\n", argv[1], argv[2]);    
	...
	res = listen(listenfd, 5);			//允许5个建立连接
